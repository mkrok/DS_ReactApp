...
_ShowTextDialog = function( title, deflt, callback )
{
    //Get orientation.
    var orient = app._160();
    ...
    var dlgTxt = app._345( title );
    var layTxtDlg = app._298( "linear", "vertical,fillxy" );
    layTxtDlg.SetPadding( 0.04, 0.02, 0.04, 0 );

    //Create dialog controls.
    txtTxtDlg = app._317( deflt, textWidth, -1, "Left" );
    layTxtDlg.AddChild( txtTxtDlg );
    
    var layTxtDlg2 = app._298( "linear", "horizontal,fillxy,center" );
    layTxtDlg2.SetMargins( 0, 0.02, 0, 0.01 ); 
   
    //Create cancel button.
    var btnTxtCancel = app._303( "Cancel", btnWidth );
    btnTxtCancel.SetOnTouch( function(){dlgTxt.Dismiss()} );
    layTxtDlg2.AddChild( btnTxtCancel );
    
    //Create ok button.
    var btnTxtOK = app._303( "OK", btnWidth );
...
_ShowTip = function( msg, left, top, timeOut, options )
{
    ...
    //Create dialog window.
    _dlgTip = app._345( null, "AutoCancel,NoDim,"+options );
    _dlgTip.SetBackColor( "#00000000" );
    _dlgTip.SetPosition( left, top );
    
    //Create tip layout.
    var layTipDlg = app._298( "linear", "vertical,fillxy" );
    
    //Create tip text.
    var txt = app._315( msg, -1,-1, "MultiLine,left,Html" );
...
_Wizard = function( title, width, height, callback, options )
{
    var curPage = 0;
    ...
    //Create dialog window.
    var dlg = app._345( title, options+"nocancel" ); //eg. nofocus
    dlg.SetTitle( title, "center" );
    var layDlg = app._298( "linear", "vertical,fillxy" );
    layDlg.SetSize( width, height );
    //layDlg.SetPadding( 0.04, 0.02, 0.04, 0 );
    
    //Create content layout.
    var layContent = app._298( "Linear", "VCenter" );
    ...
    {
        var btn = app._303( btns[i], width/4, height/8, "FontAwesome" ); 
...
_WifiScan = function( title1, title2, callback, options, extra )
{
    var self = this;
    ...
    this.Select = function( filt )
    {
        ...
        dlgScan = app._345( title1, ops );
        dlgScan.SetOnCancel( self.OnCancel );
        var lay = app._298( "linear", "vertical,fillxy" );
        //lay.SetPadding( 0.04, 0.02, 0.04, 0.02 );
    
        //Create list.
        dlgLst = app._319( "", 0.7, 0.5 );
...
_CheckList = function( title, list, callback, width, height, options )
{
    var self = this;
    ...
    this.Show = function( filt )
    {
        ...
        var maxWidth = 0;
        for( var l in list ) { 
            var w = app._294(list[l].title).width;
            if( w > maxWidth ) maxWidth = w;
        }
        
        //Create dialog window and layout.
        //var ops = (options.indexOf("force")>-1?"NoCancel":"AutoCancel");
        dlg = app._345( title, options )//options+"Kiosk" );
        
        var w = width>-1 ? width : maxWidth + TW("W")*14;
        var h = height>-1?height:-1;
        var lay = app._298( "linear", "vertical,fillxy,Left" );
        ...
        lst1 = app._319( toList(list), w, h-btnHeight-titleHeight, "Horiz,ShowScroll" );
        ...
        //var btnWidth = -1
        var layBtns = app._298( "linear", "horizontal,center" );
